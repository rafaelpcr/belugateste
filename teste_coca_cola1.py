...
def convert_radar_data(raw_data):
    ...
    try:
        ...
        converted_data = {
            ...
            'serial_number': raw_data.get('serial_number', 'RADAR_2')
        }
...
class ShelfManager:
    def __init__(self):
        # Para este caso, assumimos que a gôndola possui 1 metro de altura
        # e vamos dividi-la em 2 seções pelo eixo y.
        self.total_width = 1.0  # largura total da gôndola (pode ser ajustada conforme necessário)
        self.total_height = 1.0  # altura total da gôndola (1 metro)
    ...
    def initialize_database(self, db_manager):
        ...
        try:
            ...
            # Criar seções padrão, caso não exista nenhuma na tabela.
            self.create_default_sections(db_manager)
    ...
    def create_default_sections(self, db_manager):
        """Cria 2 seções padrão para uma gôndola com altura total de 1 metro,
           dividindo-a verticalmente em duas partes pelo eixo y.
           A primeira seção ocupará y de 0.0 a 0.5 e a segunda de 0.5 a 1.0.
        """
        try:
            # Verifica se já existem seções cadastradas
            db_manager.cursor.execute("SELECT COUNT(*) as count FROM shelf_sections")
            result = db_manager.cursor.fetchone()
            if result and result['count'] == 0:
                sections = [
                    {
                        'section_name': 'Seção Inferior',
                        'x_start': 0.0,
                        'y_start': 0.0,
                        'x_end': self.total_width,   # consideramos largura total da gôndola
                        'y_end': 0.5,
                        'product_id': 'PROD_1',
                        'product_name': 'Produto 1'
                    },
                    {
                        'section_name': 'Seção Superior',
                        'x_start': 0.0,
                        'y_start': 0.5,
                ...
                for sec in sections:
                    self.add_section(sec, db_manager)
                db_manager.conn.commit()
                logger.info("✅ Seções padrão criadas com sucesso.")
        except Exception as e:
            logger.error(f"Erro ao criar seções padrão: {str(e)}")
            logger.error(traceback.format_exc())
...
class DatabaseManager:
    ...
    def initialize_database(self):
        ...
        try:
            ...
            self.cursor.execute("""
                ...
                VALUES ('RADAR_2', 'Radar Principal', 'RADAR')
            """)
    ...
    def insert_data(self, data, analytics_data=None):
        ...
        try:
            ...
            serial_number = data.get('serial_number', 'RADAR_2')
            ...
            params = (
                ...
                float(data.get('satisfaction_score', 0)) if data.get('satisfaction_score') is not None else None,
                ...
                data.get('section_id'),
                data.get('product_id'),
                data.get('serial_number', 'RADAR_2')
            )
